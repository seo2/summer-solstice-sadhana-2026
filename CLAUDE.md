...
Before changing Next-specific APIs, routing, metadata, PWA, image handling, or static export behavior, read the relevant docs. Next used to bundle them under `node_modules/next/dist/docs/`, but the installed 15.5.x has no such folder (checked 2026-09-08), so use the version-pinned online docs instead:

```bash
open https://nextjs.org/docs/15
```

Heed deprecation warnings. `AGENTS.md` carries a warning block generated by Next itself that still points at the bundled folder — do not edit that block by hand.
